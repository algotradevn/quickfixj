...
public class SessionAdmin extends NotificationBroadcasterSupport implements SessionAdminMBean, MBeanRegistration, SessionStateListener {
	...
	public SessionAdmin(final Session session, final ObjectName connectorName, final ObjectName settingsName) {
	...
	public String getRemoteIPAddress() {
		final String remoteAddress = session.getRemoteAddress();
	 ...
	 */
	public String[] getMessages(final int startSequence, final int endSequence) throws IOException {
	...
	@Override
	public void resetSequence(final int nextSeqNum) throws SessionNotFound {
		logInvocation("resetSequence to: " + nextSeqNum);
		final Message sequenceReset = new Message();
	 ...
	 */
	protected void doSend(final Message message, final SessionID sessionID) throws SessionNotFound {
	...
	private void logAttributeChange(final String attributeName, final int value) {
		session.getLog().onEvent("JMX: setting " + attributeName + " to " + value);
	}

	private void logInvocation(final String operation) {
	...
	@Override
	public void onResendRequestSent(final int beginSeqNo, final int endSeqNo, final int currentEndSeqNo) {
		sendNotification("resendRequestSent");
	}

	@Override
	public void onSequenceResetReceived(final int newSeqNo, final boolean gapFillFlag) {
		sendNotification("sequenceResetReceived");
	}

	@Override
	public void onResendRequestSatisfied(final int beginSeqNo, final int endSeqNo) {
	...
	private void sendNotification(final String eventName) {
		final Notification notification = new Notification(NOTIFICATION_TYPE, this,
